...
Eigen::VectorXd MeasurementGyroscope::simulate(const Eigen::VectorXd & x, const SystemEstimator &) const
{
    return x.segment<3>(SystemLocalisation::iOmega) + x.segment<3>(SystemLocalisation::iGyroBias);
}

double MeasurementGyroscope::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator &) const
{
    return logLikelihoodImpl<double>(x);
}

double MeasurementGyroscope::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator &,
...
Eigen::VectorXd MeasurementBodyVelocity::simulate(const Eigen::VectorXd & x, const SystemEstimator &) const
{
    return x.segment<3>(SystemLocalisation::iVel);
}

double MeasurementBodyVelocity::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator &) const
{
    return logLikelihoodImpl<double>(x);
}

double MeasurementBodyVelocity::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator &,
                                              Eigen::VectorXd & g) const
{
    using autodiff::dual;
    using autodiff::gradient;
