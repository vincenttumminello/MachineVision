 ...
 * upstream Mahony filter (whose bias integrator is driven by the gravity error,
 * a cross product of two near-vertical vectors, which has no component about the
 * vertical), so nothing in the system was estimating the yaw-rate bias -- the
 * one that turns into steady heading drift.
 */
#ifndef MEASUREMENTBODYRATES_H
#define MEASUREMENTBODYRATES_H
...
public:
    ...
    {
        const Eigen::Vector3<Scalar> yhat = x.segment(SystemLocalisation::iOmega, 3)
                                          + x.segment(SystemLocalisation::iGyroBias, 3);
 ...
 *  - a zero-velocity update while the robot is on the ground. A fallen robot is
 *    not translating, and saying so is both the most confident measurement in the
 *    system and the only thing stopping the pre-fall velocity from integrating
 *    across the whole fall.
 */
...
public:
    ...
    Scalar logLikelihoodImpl(const Eigen::VectorX<Scalar> & x) const
    {
        const Eigen::Vector3<Scalar> e = y_.cast<Scalar>() - Eigen::Vector3<Scalar>(x.segment(SystemLocalisation::iVel, 3));
        const double sigma2 = sigma_*sigma_;
        return Scalar(-1.5*std::log(2.0*M_PI*sigma2)) - Scalar(0.5)*e.squaredNorm()/Scalar(sigma2);
    }
