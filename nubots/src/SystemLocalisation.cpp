...
static Eigen::VectorX<Scalar> dynamicsLocalisationTemplated(const Eigen::VectorX<Scalar> & x, const Eigen::VectorXd & u)
{
    ...
    f.segment(SystemLocalisation::iQuat, 4) =
        Scalar(0.5)*quatXi(q)*omegaBb.cast<Scalar>();
...
Eigen::VectorXd SystemLocalisation::input(double t, const Eigen::VectorXd & x) const
{
    ...
    if (disturbed_)
    {
        ...
        // ground, and during a getup describes a scripted flail that is not
        // locomotion; worse, twistFromOdometry drops intervals longer than maxGap,
        // so the zero-order hold above would carry the last pre-fall velocity
        // across a sensor stall. None of that is information, and it does not
        // become information after a couple of seconds -- this is keyed off the
        // posture itself, NOT off the bounded window that governs the elevated
        // PSDs. omegaBb comes from the gyroscope, which measures the topple
        // honestly, so it is kept.
        u.head<3>().setZero();
    }
    return u;
}
...
GaussianInfo<double> SystemLocalisation::processNoiseDensity(double dt) const
{
    ...
    else
    {
        sigma << params.sigmaPosXY, params.sigmaPosXY, params.sigmaPosZ,
                 quaternionSigma(params.sigmaAtt, params.sigmaYaw),
                 params.sigmaCamBias, params.sigmaCamBias;
    }
...
static GaussianInfo<double> projectQuaternion(const GaussianInfo<double> & g)
{
    ...
    if (!(n > 1e-9)) return g;      // Degenerate: leave it for the norm prior to pull back
...
void SystemLocalisation::addSideLogEvidence(double logRatio)
{
    ...
    for (std::size_t i = 0; i < components_.size(); ++i)
    {
        ...
        if (d < bestDist) { bestDist = d; repIdx = i; }
    }
...
void SystemLocalisation::setRepresentative()
{
    ...
    {
        best = std::distance(logWeights_.begin(),
            std::max_element(logWeights_.begin(), logWeights_.end()));
    }
...
void SystemLocalisation::process(Event & event)
{
    ...
    // downstream diagnostics (numAssociated, the viewer's residual lines) reflect
    // the pose actually reported rather than whichever component happened to be
    // processed last.
    if (meas) meas->reassociate(*this);
}
