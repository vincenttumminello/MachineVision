 ...
 * the gyroscope bias is estimated rather than calibrated by heuristic. The
 * process model is the kinematics plus a random walk on the rates:
 *
...
public:
    ...
    struct Parameters
    {
        ...
        // happening, so MeasurementBodyVelocity is suppressed and nothing observes
        // vBb except the zero-velocity update once the robot has settled. These PSDs
        // are what the belief decays towards meanwhile, so a fall widens honestly
        ...
        double sigmaPosXYDisturbed = 0.20;  ///< Horizontal position process noise PSD while not upright [m/sqrt(s)]
        double sigmaPosZDisturbed  = 0.20;  ///< Vertical position process noise PSD while not upright [m/sqrt(s)]
        double sigmaAttDisturbed   = 0.20;  ///< Roll/pitch process noise PSD while not upright [rad/sqrt(s)]
        double sigmaYawDisturbed   = 0.20;  ///< Yaw process noise PSD while not upright [rad/sqrt(s)]
    ...
    GaussianInfo<double> orientationDensity() const;    ///< Marginal density of the attitude quaternion
    GaussianInfo<double> velocityDensity() const;       ///< Marginal density of vBb

    /// @brief Body-fixed linear velocity of a state [m/s].
    static Eigen::Vector3d bodyVelocity(const Eigen::VectorXd & x) { return x.segment<3>(iVel); }

    /// @brief Body-fixed angular velocity of a state [rad/s].
    static Eigen::Vector3d bodyRate(const Eigen::VectorXd & x) { return x.segment<3>(iOmega); }

    /// @brief Estimated gyroscope bias of a state [rad/s].
    static Eigen::Vector3d gyroBias(const Eigen::VectorXd & x) { return x.segment<3>(iGyroBias); }

    /// @brief Speed over ground implied by the body velocity [m/s].
    static double groundSpeed(const Eigen::VectorXd & x) { return x.segment<2>(iVel).norm(); }
     ...
     *
     * Two different things follow from a fall, and they are deliberately NOT the
     * same switch, because they expire differently:
     *
     *  - The twist input's linear velocity is discarded. It is derived by
     *    differencing walk-engine odometry, which while the robot is on the ground
     *    describes a gait that is not happening -- and during a getup describes a
     *    scripted flail that is happening but is not locomotion. That is a
     *    statement about whether the signal means anything, and it does not become
     *    true again after some number of seconds: it holds for the WHOLE time the
     *    robot is not upright. The gyroscope-derived angular velocity is kept
     *    throughout, because it measures the topple for real.
     *
     *  - The process noise switches to the `*Disturbed` PSDs, so the belief decays
     *    honestly instead of coasting at walking-grade confidence. That one IS
     *    bounded: a fall is a bounded event, and modelling a robot lying still as a
     *    0.40 m/sqrt(s) random walk would make the belief's width report how long
     *    it had been down rather than how far it could have gone. It applies for
     *    params.disturbedWindow seconds and then stands down.
     *
     * Tying both to the same window is what let a long fall integrate the getup's
     * odometry at nominal confidence, marching the estimate away with a covariance
     * too tight for the association gate to ever recover it.
     *
     * This is a mode, not an event: the caller sets it every frame from the posture.
     *
     * @param upright True when the robot is upright
     * @param disturbedFor Seconds since the current non-upright episode began
     *                     (ignored when upright; 0 means "just started")
     */
     ...
     */
    bool diffusing() const { return diffusing_; }
