...
Eigen::VectorXd MeasurementKinematicHeight::simulate(const Eigen::VectorXd & x, const SystemEstimator & system) const
{
    Eigen::VectorXd y(1);
    y << x(2);
    return y;
...
double MeasurementKinematicHeight::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator & system, Eigen::VectorXd & g) const
{
    const double sigma2 = sigma_*sigma_;
...
double MeasurementKinematicHeight::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator & system, Eigen::VectorXd & g, Eigen::MatrixXd & H) const
{
    const double sigma2 = sigma_*sigma_;
