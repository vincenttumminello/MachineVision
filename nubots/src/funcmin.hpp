 ...
 * 
 * This file contains various optimization algorithms for function minimization,
 * including Newton's method, BFGS, and Levenberg-Marquardt variants.
 */
...
int BFGSLMSqrt(Func costFunc, Eigen::VectorXd & x, Eigen::VectorXd & g, Eigen::MatrixXd & Xi, int verbosity = 0)
{
    ...
    for (int i = 0; i < maxIterations; ++i)
    {
        ...
        if (verbosity == 3)
            std::printf("Iter = %5i, Cost = %10.2e, Newton decr^2 = %10.2e, Lambda = %10.2e\n", i, f, NewtonDecrSq, lambda);
        if (verbosity == 1)
            std::printf(".");
