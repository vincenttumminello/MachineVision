...
MeasurementFieldLines::MeasurementFieldLines(double time, const LinePointsSample & sample, const Pose<double> & Tbc,
    ...
    , options_(options)
{
    ...
    const std::size_t stride = std::max<std::size_t>(1, (usable.size() + options_.maxPoints - 1)/options_.maxPoints);
    ...
    for (std::size_t k = 0; k < selected.size(); ++k)
    {
        ...
        sigma2_(static_cast<Eigen::Index>(k)) = options_.sigmaDistance*options_.sigmaDistance + sigmaRange*sigmaRange;
    }
}
...
double MeasurementFieldLines::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator & system, Eigen::VectorXd & g) const
{
    using autodiff::dual;
...
double MeasurementFieldLines::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator & system, Eigen::VectorXd & g, Eigen::MatrixXd & H) const
{
