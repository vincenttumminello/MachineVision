/**
 * @file MeasurementQuaternionNorm.h
 * @brief Unit-norm pseudo-measurement pinning the redundant quaternion DOF.
 */
#ifndef MEASUREMENTQUATERNIONNORM_H
#define MEASUREMENTQUATERNIONNORM_H
...
public:
     ...
     */
    explicit MeasurementQuaternionNorm(double time, double sigma = 1e-3)
        : Measurement(time, 0)
        , sigma_(sigma)
    {
    ...
    virtual Eigen::VectorXd simulate(const Eigen::VectorXd & x, const SystemEstimator &) const override
    {
        return Eigen::VectorXd::Constant(1, x.segment<4>(SystemLocalisation::iQuat).norm());
    }

    virtual double logLikelihood(const Eigen::VectorXd & x, const SystemEstimator &) const override
    {
