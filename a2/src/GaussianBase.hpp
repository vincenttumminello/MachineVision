...
public:
    ...
    static double chi2inv(double p, double nu)
    {
        ...
        return 2 * boost::math::gamma_p_inv(nu/2, p);
    }
    ...
    {
        return 0.5 * std::erfc(-w/std::sqrt(2));
    }
    ...
    Eigen::Matrix<Scalar, 2, Eigen::Dynamic> confidenceEllipse(double nSigma = 3.0, int nSamples = 100) const
    {
        ...
        // probability enclosed by +/- nSigma
        double c_double = 2.0 * normcdf(nSigma) - 1.0;
        double chi2_double = chi2inv(c_double, 2.0);
        Scalar radius = Scalar(std::sqrt(chi2_double));

        Eigen::Matrix<Scalar, 2, Eigen::Dynamic> X(2, nSamples);

        // generate N equally spaced angles: k = 0..N-1
        for (int k = 0; k < nSamples; ++k) { // This doesn't have to be a for loop
            double theta = 2.0 * std::numbers::pi * static_cast<double>(k) / (static_cast<double>(nSamples) - 1);
            Eigen::Matrix<Scalar,2,1> w;
            w(0) = Scalar(std::cos(theta)) * radius;
            w(1) = Scalar(std::sin(theta)) * radius;

            // forward mapping: X = mu + S^T * w
            X.col(k) = mean() + sqrtCov().transpose() * w;
        }
